...
The Entity-Relationship Diagram (ERD) is rendered using Mermaid `erDiagram` syntax, employing Crow's Foot notation to represent cardinality and participation constraints accurately. Entities, attributes, and relationships are derived directly from the Business Requirement Analysis (BRA) provided in Step 1. Many-to-many relationships (like Space and Facility) are modeled as direct M:N relationships, deferring the creation of associative tables to the logical design phase. Multi-role relationships involving the `User` entity are explicitly modeled as distinct, labeled relationships to ensure clear traceability for each role (requester, approver, reporter, assigned staff, etc.).
...
|---|---|---|---|---|---|
| 1 | User_Requests_Booking | User | (0,N) : (1,1) | Booking | User submits, Booking requested by |
| 2 | User_Approves_Booking | User | (0,N) : (0,1) | Booking | Staff approves, Booking approved by |
| 3 | Space_Hosts_Booking | Space | (0,N) : (1,1) | Booking | Space hosts, Booking for Space |
| 4 | Space_Contains_Facility | Space | (0,M) : (0,N) | Facility | M:N relationship |
| 5 | Booking_Has_UsageSession | Booking | (0,1) : (1,1) | UsageSession | Booking tracked by Session |
| 6 | User_ChecksIn_UsageSession | User | (0,N) : (1,1) | UsageSession | Staff checks-in, Session checked-in by |
| 7 | User_ChecksOut_UsageSession | User | (0,N) : (0,1) | UsageSession | Staff checks-out, Session checked-out by |
| 8 | Space_Requires_Maintenance | Space | (0,N) : (1,1) | MaintenanceRecord | Space requires, Record for Space |
| 9 | User_Reports_Maintenance | User | (0,N) : (1,1) | MaintenanceRecord | User reports, Record reported by |
| 10| User_Assigned_To_Maintenance| User | (0,N) : (0,1) | MaintenanceRecord | Staff assigned, Record assigned to |
