...
| Input | Role |
|---|---|
| Step 5 DDL (`05-db-definition-Group02.sql`) | Authoritative column names, types, CHECK values, FK relationships, IDENTITY columns |
| Step 1 BRA (`01-business-req-analysis-Group02.md`) | Business rules (§7), assumptions (§8), actor roles, status enumerations |
| Step 3 Logical Design (`03-logical-design-Group02.md`) | Nullability rules, referential integrity actions, FK dependency order |
| Step 6 Sample Data (`06-sample-data-Group02.sql`) | The INSERT script under review |
...
Before running any check, the reviewer must build two reference structures from the source documents:

**3a. Schema Reference** (from DDL):
For every table: column names, types, NOT NULL/NULL status, CHECK allowed values, DEFAULT values, PK definition, FK definitions, IDENTITY columns.

**3b. Business Rule Reference** (from BRA §7 and §8):
List every business rule that has a structural or data implication. For each rule, note:
...
These two references are used in every check below. The reviewer must not rely on memory for schema or rule details.
...
1. List all allowed values exactly as written in the CHECK constraint.
2. List all values actually used in the INSERT script for that column.
...
Any value in the "Values Used" column that does not exactly match an allowed value is a **FAIL** — it will cause a runtime INSERT error.
...
**Procedure:**
From the DDL, extract every FK relationship. For every FK column in every child table, track all PK values inserted into the parent table and verify that every non-NULL FK value in the child table matches an existing parent PK.
...
| **Approver/actor role constraints** | Any field that must reference a user of a specific role (e.g., approver must be staff/manager) references a user with the correct role |
...
**Procedure:**
For every column with an enumerated CHECK constraint, verify that the sample data includes at least one row using each allowed value. Produce this table:

| Table | Column | Allowed Values | Values Present in Data | Missing Values |
|---|---|---|---|---|
| ... | ... | ... | ... | ... |

**Pass condition:** All allowed values appear at least once. Missing values are a WARN (reduces query usefulness) unless the missing value is required by a specific business scenario, in which case it is a FAIL.
...
### Check 11 — SQL Syntax and Executability
...
**Pass condition:** No syntax issues that would cause runtime errors detected. Any issue causing a runtime failure is a FAIL.
...
>
> `docs/06-sample-data-review-Group02.md`
>
...
| 11 | SQL Syntax & Executability | PASS/WARN/FAIL | <n> issues |
...
[Include the mandatory output tables for Check 3, Check 4, Check 5, Check 6 conflict pairs, and Check 7 scenario matrix.]
...
| **REQUIRES REVISION** | Any check FAIL, or any BLOCKING issue found regardless of check result. |

If the verdict is REQUIRES REVISION, the reviewer must:
1. List every blocking issue with row-level correction instructions.
2. Produce corrected INSERT statements for every affected table.
...
1. **Check 3:** The full CHECK constraint compliance table must be present, listing all allowed values and all values found in the data for every constrained column.
...
4. **Check 6:** For any conflict-prevention rule (e.g., no overlapping bookings), every relevant record pair must be enumerated and evaluated — not summarised.
...
- Omit any of the five mandatory exhibits.
